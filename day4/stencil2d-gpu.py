...
#tmp_field = None
alpha = 1.0 / 32.0
conv_ker = -alpha * np.array([[0, 0,1., 0,0],
                              [0, 2,-8, 2,0],
                              [1,-8,20,-8,1],
                              [0, 2,-8, 2,0],
                              [0, 0, 1, 0,0]])
conv_ker[2,2] += 1 #I-alpha*lap^2
conv_ker2 = np.array([[0,1.,0],# lap
                      [1,-4,1],
                      [0,1.,0]])
conv_ker3 = -alpha*conv_ker2
conv_ker3[1,1] += 1 #I-alpha*lap
...
def apply_diffusion(in_field, out_field, alpha, num_halo, num_iter=1):
    ...
    for n in range(num_iter):
        ...
        #)
        #ndimage.convolve(in_field, conv_ker[np.newaxis,:,:], out_field, mode = "constant", cval = 0.0)
        ndimage.convolve(in_field, conv_ker2[np.newaxis,:,:], tmp_field, mode = "constant", cval = 0.0)
        ndimage.convolve(tmp_field, conv_ker3[np.newaxis,:,:], out_field, mode = "constant", cval = 0.0)
        if n < num_iter - 1:
